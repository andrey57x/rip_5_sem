...
func UploadImage(ctx context.Context, client *minio.Client, bucket string, file *multipart.FileHeader, reactionID int) (string, error) {
	f, err := file.Open()
	if err != nil {
		return "",err
	}
	...
	objectName := fmt.Sprintf("%d%s", reactionID, ext)
...
func ListObjects(ctx context.Context, client *minio.Client, bucket, prefix string) error {
	...
	for obj := range client.ListObjects(ctx, bucket, opts) {
		...
		fmt.Println("object:", obj.Key, " size:", obj.Size, " lastModified:", obj.LastModified)
	}
	return nil
}
